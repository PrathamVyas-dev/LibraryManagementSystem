...
namespace FinalProject.Application.Interfaces
{
    public interface INotificationService
    {
        ...
        Task UpdateNotificationAsync(int notificationId, CreateNotificationDto updateDto);
        Task DeleteNotificationAsync(int notificationId);
        Task<IEnumerable<NotificationDetailsDto>> GetNotificationsForMemberAsync(int memberId);
        Task<IEnumerable<NotificationDetailsDto>> GetNotificationsByMemberNameAsync(string memberName);
        ...
        Task NotifyForMembershipStatusChangeAsync(int memberId, string newStatus);
        Task PerformPeriodicChecksAsync(); // Add method for periodic checks
    }
}
