...
namespace FinalProject.Application.Services
{
    public class NotificationService : INotificationService
    {
        ...
        public async Task NotifyForDueBooksAsync()
        {
            var transactions = await _borrowingRepository.GetAllTransactionsAsync();
            foreach (var transaction in transactions)
            {
                var daysUntilDue = (transaction.ReturnDate - DateTime.Now).Days;
                if (daysUntilDue > 0 && daysUntilDue <= 4)
                {
                    var message = $"Your book is due in {daysUntilDue} days.";
