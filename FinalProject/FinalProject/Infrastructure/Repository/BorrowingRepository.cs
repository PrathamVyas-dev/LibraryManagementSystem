...
namespace FinalProject.Infrastructure.Repository
{
    public class BorrowingTransactionRepository : IBorrowingTransactionRepository
    {
        ...
        {
            // Only consider transactions that are not returned and have a BorrowDate + 14 days < now
            var now = DateTime.Now;
            return await _context.BorrowingTransactions
                .Where(t => t.Status != "Returned" 
                            && t.ReturnDate == DateTime.MinValue // Not yet returned
                            && t.BorrowDate.AddDays(14) < now)
                .ToListAsync();
        }
        ...
        {
            return await _context.BorrowingTransactions
                .FirstOrDefaultAsync(t => t.MemberID == memberId && t.BookID == bookId && t.Status == "Borrowed");
        }
    }
}