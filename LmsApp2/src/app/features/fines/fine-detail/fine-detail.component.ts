...
export class FineDetailComponent implements OnInit {
  ...
  loadFineDetails(fineId: number): void {
    ...
    this.fineService.getFineById(fineId).subscribe({
      next: (fine) => {
        ...
        if (fine.memberID) {
          this.loadMemberName(fine.memberID);
        }
  ...
  loadMemberName(memberId: number): void {
    // If you have a way to get member name from another service or API, implement it here
  ...
  loadMemberDetails(memberId: number): void {
    this.memberService.getMember(memberId).subscribe({
      ...
      error: () => {
        // Silently fail - we already have the fine data
      }
    });
  }
