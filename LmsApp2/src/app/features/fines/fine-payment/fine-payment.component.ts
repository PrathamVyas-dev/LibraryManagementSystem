...
export class FinePaymentComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    // Get full UI form values (for UI display/validation)
    const formValues: FinePaymentUiModel = this.paymentForm.value;
    ...
    console.log('Submitting payment:', paymentDto);
  ...
  processFinePayment(paymentDto: PayFineDto): void {
    this.loading = true;
    this.error = null;
