...
export class FineService {
  ...
  mapToFineDisplayUiModel(fineDto: FineDetailsDto, memberName?: string): FineDisplayUiModel {
    const displayModel: FineDisplayUiModel = {
    ...
    // Calculate days overdue for UI purposes
    if (fineDto.status === 'Pending') {
      const creationDate = new Date(fineDto.transactionDate);
      const today = new Date();
      const diffTime = Math.abs(today.getTime() - creationDate.getTime());
      displayModel.daysOverdue = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    }
